...
/**
 * Sync openclaw config, skills, and workspace from container to R2 for persistence.
 * 
 * This function:
...
export async function syncToR2(sandbox: Sandbox, env: OpenClawEnv): Promise<SyncResult> {
  ...
  const syncCmd = [
    ...
    `rsync -r --no-times --delete /root/openclaw/skills/ ${R2_MOUNT_PATH}/skills/`,
    `rsync -r --no-times --delete --exclude='skills' --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/openclaw/ ${R2_MOUNT_PATH}/workspace/`,
  ...
  try {
    ...
    // Note: backup structure is ${R2_MOUNT_PATH}/openclaw/, ${R2_MOUNT_PATH}/skills/, and ${R2_MOUNT_PATH}/workspace/
