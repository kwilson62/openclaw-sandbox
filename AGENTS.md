...
1. `openclaw.json.template` is copied to `~/.openclaw/openclaw.json` (internal path unchanged)
...
| `OPENCLAW_GATEWAY_TOKEN` | `--token` flag | |
| `OPEN_DEV_MODE` | `controlUi.allowInsecureAuth` | Mapped from `DEV_MODE` |
